...
////////////Reading from root file
    ...
    //Double_t 0[18] = {0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5};

////////////Drawing graphs
    TGraphErrors* numMuons = new TGraphErrors(18);

    Int_t numMuonsArr[18];
    ...
    for (int i=0; i<18; i++) 
    {
        ...
        Int_t num = ntMuons->GetEntries(req);
        numMuons->SetPoint(i,xArr[i],num);
        numMuonsArr[i] = num;
    }
////// a: Charm Quark Parent
    ...
    for (TH1F* hist: allCleanHists) 
    {
        ...
        hist->Draw();
        hist->Scale(1./numMuonsArr[count],"width");
...
////Widths
    ...
    mg->Draw("AP");
    mg->GetXaxis()->SetTitle("p_{t}^{#mu} (GeV)");
...
////Integrals
    ranges->cd(2);
    //TCanvas* ranges2 = new TCanvas("ranges2","Comparison: electron momentum range vs histogram2");
...
// 6: Effect of ptMu cut on number of pairs
/*
